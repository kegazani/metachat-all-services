...
import (
	...
	grpcServer "metachat/match-request-service/internal/grpc"
...
func main() {
	...
	if err := matchRequestRepo.InitializeTables(); err != nil {
		logger.Fatalf("Failed to initialize database tables: %v", err)
	}

	matchRequestService := service.NewMatchRequestService(matchRequestRepo, logger)
	grpcSrv := grpcServer.NewMatchRequestServer(matchRequestService, logger)
	...
	s := grpc.NewServer()
	pb.RegisterMatchRequestServiceServer(s, grpcSrv)
