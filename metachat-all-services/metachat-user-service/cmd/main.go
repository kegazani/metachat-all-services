...
import (
	...
	"metachat/user-service/internal/clients"
...
func main() {
	...
	personalityServiceAddr := viper.GetString("services.personality_service_address")
	if personalityServiceAddr == "" {
		personalityServiceAddr = "localhost:50056"
	}

	analyticsServiceAddr := viper.GetString("services.analytics_service_address")
	if analyticsServiceAddr == "" {
		analyticsServiceAddr = "localhost:50057"
	}

	personalityClient, err := clients.NewPersonalityClient(personalityServiceAddr)
	if err != nil {
		logger.WithError(err).Warn("Failed to create personality client, continuing without it")
		personalityClient = nil
	} else if personalityClient != nil {
		defer personalityClient.Close()
	}

	analyticsClient, err := clients.NewAnalyticsClient(analyticsServiceAddr)
	if err != nil {
		logger.WithError(err).Warn("Failed to create analytics client, continuing without it")
		analyticsClient = nil
	} else if analyticsClient != nil {
		defer analyticsClient.Close()
	}

	userService := service.NewUserService(userRepository, userReadRepository, userEventProducer, jwtManager, googleOAuth, appleOAuth, personalityClient, analyticsClient)

	// Initialize Kafka consumer for personality events
	consumerGroupID := viper.GetString("kafka.consumer_group")
	if consumerGroupID == "" {
	...
	var personalityConsumer kafka.PersonalityEventConsumer
	var cancelConsumer context.CancelFunc
	if consumer, err := kafka.NewPersonalityEventConsumer(
		kafkaBootstrapServers,
		consumerGroupID,
		personalityTopics,
		userReadRepository,
		...
		logger.WithError(err).Warn("Failed to create personality event consumer, continuing without it")
	} else {
		personalityConsumer = consumer
		consumerCtx, cancel := context.WithCancel(context.Background())
		cancelConsumer = cancel

		if err := personalityConsumer.Start(consumerCtx); err != nil {
			logger.WithError(err).Warn("Failed to start personality event consumer, continuing without it")
			cancelConsumer = nil
		} else {
			logger.Info("Personality event consumer started")
		}
	}
	...
	if cancelConsumer != nil {
		cancelConsumer()
		if personalityConsumer != nil {
			personalityConsumer.Stop()
		}
	}
