...
func (r *diaryReadRepository) processDiaryEntryCreatedEvent(ctx context.Context, event *events.Event) error {
	...
	if payload.UserID == "" {
		return fmt.Errorf("user_id is missing in event payload")
	}
	...
	if entry.UserID != payload.UserID {
		return fmt.Errorf("user_id mismatch: entry.UserID=%s, payload.UserID=%s", entry.UserID, payload.UserID)
	}
	...
	if entryByUser.UserID != payload.UserID {
		return fmt.Errorf("user_id mismatch in entryByUser: entryByUser.UserID=%s, payload.UserID=%s", entryByUser.UserID, payload.UserID)
	}
	...
	if entryByUserAndTime.UserID != payload.UserID {
		return fmt.Errorf("user_id mismatch in entryByUserAndTime: entryByUserAndTime.UserID=%s, payload.UserID=%s", entryByUserAndTime.UserID, payload.UserID)
	}
