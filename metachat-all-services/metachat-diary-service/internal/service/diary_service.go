...
func (s *diaryService) CreateDiaryEntry(ctx context.Context, userID, title, content string, tokenCount int, sessionID string, tags []string) (*aggregates.DiaryAggregate, error) {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		uncommittedEvents := diary.GetUncommittedEvents()
		for _, event := range uncommittedEvents {
			var payload events.DiaryEntryCreatedPayload
			if err := event.UnmarshalPayload(&payload); err == nil {
				logrus.WithFields(logrus.Fields{
					"event_user_id": payload.UserID,
					"request_user_id": userID,
					"aggregate_id": event.AggregateID,
				}).Info("Diary entry event created")
			}
		}
		...
		if err = s.publishDiaryEvents(ctx, uncommittedEvents); err != nil {
