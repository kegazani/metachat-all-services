...
func (s *DiaryServer) ListDiaryEntries(ctx context.Context, req *pb.ListDiaryEntriesRequest) (*pb.ListDiaryEntriesResponse, error) {
	...
	if req.Filter == "" {
		s.logger.Warn("ListDiaryEntries called without filter - returning empty list")
		return &pb.ListDiaryEntriesResponse{
			Entries: []*pb.DiaryEntry{},
			Total:   0,
			Page:    req.Page,
			Limit:   req.Limit,
		}, nil
	...
	if endIdx > len(readModels) {
		endIdx = len(readModels)
	}

	paginatedReadModels := readModels[startIdx:endIdx]
	protoEntries := make([]*pb.DiaryEntry, len(paginatedReadModels))
	for i, entry := range paginatedReadModels {
		protoEntries[i] = s.diaryEntryReadModelToDiaryEntryProto(entry)
	}
...
func (s *DiaryServer) GetDiaryEntriesByUser(ctx context.Context, req *pb.GetDiaryEntriesByUserRequest) (*pb.GetDiaryEntriesByUserResponse, error) {
	...
	readModels, err := s.diaryService.GetDiaryEntryReadModelsByUserID(ctx, req.UserId)
	if err != nil {
...
func (s *DiaryServer) diaryEntryReadModelToDiaryEntryProto(entry *models.DiaryEntryReadModel) *pb.DiaryEntry {
	return &pb.DiaryEntry{
		Id:         entry.ID,
		UserId:     entry.UserID,
		Title:      entry.Title,
